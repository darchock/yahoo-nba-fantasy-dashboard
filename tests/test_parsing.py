...
class TestCacheMetadata:
    ...
    def test_format_cache_metadata_with_cache(self):
        """Test format_cache_metadata with cached data."""
        ...
        result = format_cache_metadata(cache)
...
class TestSmartCaching:
    """Tests for smart caching with lazy refresh strategy."""
    ...
    def test_save_cached_data_complete_week(self):
        """Test save_cached_data marks complete weeks as is_complete=True."""
        ...
        db = next(get_db())
        try:
            # Save data for a completed week
            cache = save_cached_data(
                db=db,
                league_key="test_league_smart_cache_complete",
                data_type="test_data",
                data={"test": "value"},
                week=5,
                is_complete=True,  # Completed week
            )

            assert cache.is_complete is True
            assert cache.is_stale is False  # Complete data is never stale
    ...
    def test_save_cached_data_current_week(self):
        """Test save_cached_data defaults to is_complete=False."""
        from backend.routes.api import save_cached_data
        from app.database.connection import get_db
        ...
        db = next(get_db())
        try:
            # Save data for current week (default is_complete=False)
            cache = save_cached_data(
                db=db,
                league_key="test_league_smart_cache_current",
                data_type="test_data",
                data={"test": "value"},
                week=15,
                is_complete=False,  # Current week
            )

            assert cache.is_complete is False
            # Note: is_stale depends on when it was fetched vs 6 AM Eastern
    ...
    def test_cached_data_is_stale_with_complete_flag(self):
        """Test that CachedData with is_complete=True is never stale."""
        from datetime import datetime, timedelta, timezone
        from app.database.models import CachedData

        # Data fetched yesterday but marked as complete
        old_fetch = datetime.now(timezone.utc) - timedelta(days=7)
        cache = CachedData(
            league_key="test",
            data_type="test",
            json_data={},
        ...
        # Should NOT be stale because is_complete=True
